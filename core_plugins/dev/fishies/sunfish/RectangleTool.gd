...
class RectangleElement extends ShapeTool.ShapeElement:
	...
	func draw(canvas: Whiteboard.ElementLayer, wb: Whiteboard) -> void:
		...
		canvas.draw_circle(p_tl, width * 0.5, wb.get_color(color))
		canvas.draw_circle(p_tr, width * 0.5, wb.get_color(color))
		canvas.draw_circle(p_bl, width * 0.5, wb.get_color(color))
		canvas.draw_circle(p_br, width * 0.5, wb.get_color(color))
		canvas.draw_line(p_tl, p_tr, wb.get_color(color), width)
		canvas.draw_line(p_tr, p_br, wb.get_color(color), width)
		canvas.draw_line(p_br, p_bl, wb.get_color(color), width)
		canvas.draw_line(p_bl, p_tl, wb.get_color(color), width)
