...
class FilledRectangleElement extends ShapeTool.ShapeElement:
	...
	func draw(canvas: Whiteboard.ElementLayer, wb: Whiteboard) -> void:
		...
		canvas.draw_circle(rect_pos + rect_size * Vector2(0, 0), width * 0.5, wb.get_color(color))
		canvas.draw_circle(rect_pos + rect_size * Vector2(1, 0), width * 0.5, wb.get_color(color))
		canvas.draw_circle(rect_pos + rect_size * Vector2(0, 1), width * 0.5, wb.get_color(color))
		canvas.draw_circle(rect_pos + rect_size * Vector2(1, 1), width * 0.5, wb.get_color(color))
		canvas.draw_rect(Rect2(
			rect_pos - Vector2(width * 0.5, 0.0),
			rect_size + Vector2(width, 0.0)
		).abs(), wb.get_color(color))
		canvas.draw_rect(Rect2(
